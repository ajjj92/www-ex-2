...
function initBoard() {
  let cssString = "auto ".repeat(currentSize);
  table.style.gridTemplateColumns = cssString;
  ...
  boardState.forEach(row => {
    const divCol = document.createElement("tr");
    table.appendChild(divCol);
    row.forEach(element => {
...
function processMove(clickedCell) {
  const rowIndex = clickedCell.cellIndex;
  const colIndex = clickedCell.parentElement.rowIndex;
